...
const ANALYTICS_IP_SALT = process.env.ANALYTICS_IP_SALT || null;

function hashIP(ip) {
  if (!ip || !ANALYTICS_IP_SALT) return null;
  return crypto.createHmac('sha256', ANALYTICS_IP_SALT).update(ip).digest('hex').substring(0, 16);
}
