...
const ANALYTICS_IP_SALT = process.env.ANALYTICS_IP_SALT || 'pact-analytics-salt-2026';

function hashIP(ip) {
  if (!ip) return null;
  return crypto.createHash('sha256').update(ip + ANALYTICS_IP_SALT).digest('hex').substring(0, 16);
}
