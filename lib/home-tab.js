...
// Injected via init()
let pool, formatDate, getUserTimezone;
...
async function buildHomeView(userId, client, { selectAllOverdue = false, teamId: teamIdHint = null } = {}) {
  ...
  // ── Viral invite section — shown after 3+ total pacts created
  if (stats.totalCreated >= 3 && teamId) {
    ...
    const incentiveHeadline = `*🤝 Bring another team onto Pact*`;
    const incentiveBody = `Share your link — when a teammate from another company installs Pact, they become your cross-workspace pact partner.\n\nProgress: ${progressBar}`;

    const socialText = encodeURIComponent(
      `invite teammates' workspaces to Pact and track commitments in Slack with automatic reminders → ${getAppUrl()}`
    );
