...
function getStripeProPaymentLink() {
  return process.env.STRIPE_PRO_PAYMENT_LINK || process.env.STRIPE_PAYMENT_LINK_PRO || null;
}
...
function registerBillingRoutes(app, pool) {
  ...
  app.get('/api/checkout', async (req, res) => {
    try {
      ...
      const APP_BASE = getAppUrl();
      const stripeProPaymentLink = getStripeProPaymentLink();
      if (!stripeProPaymentLink) {
        return res.status(503).json({
          error: 'Stripe checkout is not configured. Set STRIPE_PRO_PAYMENT_LINK in Vercel.',
        });
      }

      let checkoutUrl = stripeProPaymentLink;
      ...
      if (team_id) {
        ...
        checkoutUrl = `${stripeProPaymentLink}?client_reference_id=${encodeURIComponent(ref)}`;
