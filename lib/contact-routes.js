...
function registerContactRoutes(app, pool) {
  ...
  app.get('/api/contact-submissions', async (req, res) => {
    try {
      // Simple auth: require a query param token for now
      // In production this would be a proper admin auth check
      const token = req.query.token;
      const expectedToken = process.env.CONTACT_ADMIN_TOKEN || 'pact-contact-admin';
