...
function init(deps) {
  ...
  getTeamTier = access.getTeamTier;
  planBadge = access.planBadge;
  getMonthlyPactCount = (teamId) => access.getMonthlyPactCount(teamId, pool);
  PLAN_MONTHLY_LIMITS = access.PLAN_MONTHLY_LIMITS;
...
async function handleCreatePact({ command, ack, respond, client }) {
  ...
  if (text && text.trim().toLowerCase() === 'help') {
    ...
    const planNote = `${badge} — Unlimited pacts + Linear, Notion, Asana sync`;
  ...
  try {
    ...
    // 5. Legacy limit hook; all current plans are unlimited.
    ...
    if (monthlyLimit !== null) {
      ...
      if (monthlyCount >= monthlyLimit) {
        await respond(':warning: Your workspace has reached its configured pact limit.');
        return;
      }
    }
    ...
    // this marks the invite as a successful cross-workspace referral.
    require('../db/invites').markInvitePactCreated(team_id).catch(() => {});

    // Async tracker sync — non-blocking
...
// Shows the user's invite link and cross-workspace referral progress.
// ---------------------------------------------------------------------------
...
async function handleInviteCommand({ command, respond, client }) {
  ...
  try {
    ...
    // Progress toward bringing two additional workspaces into the network.
    ...
    const incentiveText = successfulCount >= 2
      ? ':trophy: *Two workspaces joined through your link!*'
      : `*Invite ${remaining} more workspace${remaining === 1 ? '' : 's'} to grow your Pact network.*`;

    const copyText = `invite teammates' workspaces to Pact. Share this link → ${link}`;
    ...
    const blocks = [
      ...
      {
        type: 'context',
        elements: [{ type: 'mrkdwn', text: '_Your link never expires · Each workspace counts once_' }],
      },
    ];

    await respond({ response_type: 'ephemeral', blocks, text: `Your Pact invite link: ${link}` });
  } catch (err) {
...
async function handleDMConfirmPact({ action, ack, body, client }) {
  ...
  try {
    ...
    // Quality signal: this workspace created a pact within 7d of install.
...
async function handleReactionConfirmPact({ action, ack, body, client }) {
  ...
  try {
      ...
      if (monthlyLimit !== null) {
        ...
        if (monthlyCount >= monthlyLimit) {
          await client.chat.update({
            ...
            ts: messageTs,
            text: `:warning: Your workspace has reached its configured pact limit.`,
            blocks: []
          });
          return;
        }
    ...
    if (resolvedTeamId) {
      ...
      // Quality signal for a successful cross-workspace referral.
...
/**
 * Analyze a channel message for commitment language and
 * send an ephemeral "Make it a pact?" prompt to the sender if a commitment is detected.
 *
 * Guards: not DMs · not bot messages · rate-limited (1/user/hr) · channel-snooze respected
 *
...
async function handleHomeHelp({ ack, body, respond }) {
  ...
  const blocks = [
    ...
    },
    {
      type: 'context',
      elements: [
        { type: 'mrkdwn', text: badge + ' — unlimited pacts + integrations' },
      ],
    },
  ];
...
function registerSlackHandlers(slackApp) {
  ...
  // AI-powered /done suggestion actions
  ...
  // ── AI Commitment Detection ───────────────────────────────────────────────
  // Listen for messages in public/private channels (not DMs) and
