...
async function handleDMHelpMessage(client, userId, channelId, teamId) {
  ...
  const planNote = helpTier === 'pro'
    ...
    : `${badge} — Unlimited pacts + integrations`;
...
function serverlessSlashCommand(handler) {
  return async (args) => {
    ...
    // On a persistent server, handlers can send an empty acknowledgement and
    // use response_url afterward. Vercel may freeze the invocation as soon as
    // that empty acknowledgement ends the HTTP request. Delay an empty ack so
    // the handler's first visible response can be returned inline instead.
