...
async function handleCreatePact({ command, ack, respond, client }) {
  ...
  if (text && text.trim().toLowerCase() === 'help') {
    ...
    const planNote = helpTier === 'pro'
      ...
      : `${badge} — Up to 100 new active pacts/month · _/pact upgrade_ for Pro early access`;
  ...
  try {
    ...
    if (monthlyLimit !== null) {
      ...
      if (monthlyCount >= monthlyLimit) {
        const upgradeMsg = `:warning: Your workspace has reached the *${monthlyLimit} newly created active pacts/month* limit on the Free plan.\n\nPro adds unlimited usage and tracker sync. Type \`/pact upgrade\` for current early-access details.`;
  ...
  try {
    ...
    if (!isPro) {
      await respond({
        ...
        blocks: [
          ...
          },
          {
            ...
            elements: [
              {
                type: 'button',
                text: { type: 'plain_text', text: 'Learn about Pro ↗', emoji: true },
                style: 'primary',
        ...
        text: 'Tracker Sync is a Pro feature. See makepact.co/#pricing for early-access details.'
      });
      return;
    }
...
async function handleUpgradeCommand({ respond, team_id, user_id }) {
  try {
    ...
    const selfServiceBillingConfigured = Boolean(
      process.env.STRIPE_PRO_PAYMENT_LINK || process.env.STRIPE_PAYMENT_LINK_PRO
    );
    if (!selfServiceBillingConfigured) {
      await respond({
        response_type: 'ephemeral',
        text: `Pro is currently in early access. Email hello@makepact.co with your workspace name to request access.\n\nCurrent plan: Free · ${usageStr}`,
      });
      return;
    }
...
async function handleBillingCommand({ respond, team_id, user_id }) {
  try {
    ...
    } else if (!(process.env.STRIPE_PRO_PAYMENT_LINK || process.env.STRIPE_PAYMENT_LINK_PRO)) {
      await respond({
        response_type: 'ephemeral',
        text: 'You are on the Free plan. Pro is currently in early access; email hello@makepact.co with your workspace name to request access.',
      });
    } else {
...
async function handleDowngradeCommand({ respond, team_id, user_id }) {
  try {
    ...
    if (currentTier !== 'pro') {
      ...
      await respond({
        response_type: 'ephemeral',
        text: selfServiceBillingConfigured
          ? `You're already on the Free plan. Use \`/pact upgrade\` when you are ready to move to Pro.`
          : `You're already on the Free plan. Pro is currently in early access; email hello@makepact.co with your workspace name to request access.`,
      });
      return;
    }
      ...
      if (monthlyLimit !== null) {
        ...
        if (monthlyCount >= monthlyLimit) {
          await client.chat.update({
            ...
            ts: messageTs,
            text: `:warning: Your workspace has hit the *${monthlyLimit} newly created active pacts/month* limit on the Free plan. Type \`/pact upgrade\` for Pro early-access details.`,
            blocks: []
          });
          return;
        }
    ...
    if (monthlyCount >= monthlyLimit) {
      try {
        await client.chat.postEphemeral({
          ...
          text: `:warning: Your workspace has reached the *${monthlyLimit} newly created active pacts/month* limit. Type \`/pact upgrade\` for Pro early-access details.`,
        });
      } catch {}
      return;
    }
...
async function handleHomeHelp({ ack, body, respond }) {
  ...
  const blocks = [
    ...
    },
    {
      type: 'context',
      elements: [
        { type: 'mrkdwn', text: badge + (helpTier === 'pro' ? ' — unlimited pacts + integrations' : ' — up to 100 new active pacts/mo · /pact upgrade for Pro early access') },
      ],
    },
  ];
