...
const requireDiagnosticsAuth = createAdminAuth({
  envVars: ['ADMIN_SECRET', 'CRON_SECRET'],
  queryKeys: ['secret'],
});

function registerSlackDiagnostics(router, pool, slackApp) {
  // Diagnostic endpoint — verify bot token and scopes
  router.get('/slack/status', requireDiagnosticsAuth, async (req, res) => {
    try {
  ...
  router.get('/slack/verify-events', requireDiagnosticsAuth, async (req, res) => {
    try {
