...
// Does NOT own: public analytics, Slack handlers, page routes
...
function registerMetricsRoutes(app, pool) {
  app.get('/api/metrics', requireAdminAuth, async (req, res) => {
    try {
      const now = new Date();
  ...
  app.get('/api/metrics/daily', requireAdminAuth, async (req, res) => {
    try {
