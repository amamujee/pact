...
async function sendEmail({ to, subject, body, html }) {
  if (process.env.RESEND_API_KEY && process.env.EMAIL_FROM) {
    return postJson('https://api.resend.com/emails', process.env.RESEND_API_KEY, {
  ...
  if (!url) {
    console.warn('[email] RESEND_API_KEY/EMAIL_FROM or EMAIL_SEND_URL not set; skipping outbound email');
    return { skipped: true };
  }
